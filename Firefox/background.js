  ...
  storeAnkiImage: (request, sender, sendResponse) => {
    (async () => {
      try {
        ...
        // Sanitise: keep alphanumerics, dots, hyphens, underscores; prefix to avoid collisions
  ...
  generateGeminiImage: (request, sender, sendResponse) => {
    (async () => {
      try {
        ...
        if (!apiKey) {
          sendResponse({ success: false, error: 'Gemini API key not configured in Settings' });
          return;
        }
        ...
        if (!response.ok) {
          ...
          // 429 with "quota" usually means billing not enabled — image generation is paid-only
        ...
        // Parse image from /v1beta/interactions response format
        let imageBase64, mimeType;
        if (json.output) {
          const part = json.output.find(p =>
            p.type === 'image' || (p.mimeType && p.mimeType.startsWith('image/'))
          );
